            ...
            <div className=" flex items-center md:space-x-4 gap-1 ">
                {navItems.map(({ label, path, icon }) => (
                    <NavLink
                        ...
                        to={path}
                        className={({ isActive }) =>
                            `flex bg-gray items-center uppercase text-xs px-6 py-3 rounded-full font-bold transition-all duration-200 ${isActive
                                ? ' text-black font-bold'
                                : 'text-gray-400  hover:text-black'
                            }`
                        }
                    >
                        <span className="md:mr-2 text-base">{icon}</span>
